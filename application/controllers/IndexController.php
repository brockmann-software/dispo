...
class IndexController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
		...
		$qm_db_inbounds = $this->db->query("SELECT count(No) as cnt_inbqm FROM dispo_live.v_inb_line where (ifs<>'' and ifs<now()) or (GG<>'' and GG < now()) or (QS<>'' and QS < now()) or (BIO<>'' and BIO<now())")->fetchAll()[0];
